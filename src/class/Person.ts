...
export class Person {
    ...
    static async update(data: never) {
        const user = data.physicalPerson ? new PessoaFisica() : new PessoaJuridica()
    ...
    static async register(socket: Socket, data: PersonForm) {
        try {
            ...
            const user = data.physical_person ? new PessoaFisica(person_prisma.id) : new Person(person_prisma.id)
